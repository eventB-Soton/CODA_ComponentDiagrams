...
public class SynchronousStatemachineTransitionGuardRule extends AbstractSynchronousStatemachineRule  implements IRule {
	...
	public List<GenerationDescriptor> fire(EventBElement sourceElement, List<GenerationDescriptor> generatedElements) throws Exception {
		...
		for (Event elaboratedEvent : ((Transition) sourceElement).getElaborates()){
			...
			//guard for state-machine synch 
			ret.add(Make.descriptor(elaboratedEvent,guards,Make.guard(Strings.SYNCSM_GUARD_NAME(rootSm), Strings.SYNCSM_FALSE_PRED(rootSm)),5));
